![scaffold](/scaffold.png)

**Designed for Scaffold\*, the peer-reviewed research journal at the John H. Daniels Faculty of Architecture, Landscape, and Design at the University of Toronto, this project is a testament to the importance of derivative work.**

Scaffold* Grotesk simultaneously draws on influences from the past and is an amalgam of what came before it. It builds directly off of Rag, an open-source typeface by Dennis Grauel, which, in turn, builds directly off of Arial, which builds on many typefaces before it. [Read more about Scaffold* Grotesk on scaffoldjournal.com](https://scaffoldjournal.com/scaffold-grotesk).

# Download
The [current version](/fonts/Scaffold*%20Grotesk%20050326) is timestamped **Scaffold\* Grotesk 050326** after being updated on 3 May 2026.
# <kbd>[**Direct Download / Scaffold\* Grotesk 050326**](https://go.hannnna.com/scaffold-grotesk)</kbd>

# Licence
This Font Software is licensed under the SIL Open Font License, Version 1.1. The licence is included in this repository, and is also available with a FAQ at [scripts.sil.org/OFL](http://scripts.sil.org/OFL). All other contributions to the project outside of the typeface, including promotional images, accompanying text and/or web articles, are subject to copyright. The logos, emblems, and coat of arms of the University of Toronto and the John H. Daniels Faculty of Architecture, Landscape, and Design are copyrighted material and may not be used without express permission from the University of Toronto.

*Note: This GitHub repo was created retroactively on 4 May 2026.*
